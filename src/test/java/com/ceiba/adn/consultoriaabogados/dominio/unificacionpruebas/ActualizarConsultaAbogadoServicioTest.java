...
public class ActualizarConsultaAbogadoServicioTest {
	...
	private CrearConsultaAbogadoServicio servicioCrear;
	...
	private static final Long ID_CONSULTA_INCORRECTO = (long) 20;
	...
	public void actualizarConsultaAbogadoNoExiste() {
		...
		ConsultaAbogado consultaActualizada = this.consultaBuilder.build();
		when(this.repositorio.crearConsulta(this.consultaAbogado)).thenReturn(consultaActualizada);
		this.servicio = new ActualizarConsultaAbogadoServicio(this.repositorio);
		try {
			ConsultaAbogado consultaRespuesta = this.servicio.actualizarConsultaAbogado(this.consultaAbogado);
			fail();
